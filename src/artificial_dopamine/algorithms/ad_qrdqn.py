...
def get_quantile_at_action(
    ...
    """Return the quantile values corresponding to the given actions."""
    def _get(s: jnp.ndarray, a: int) -> jnp.ndarray:
        return s[:, a]
    return jax.vmap(_get)(quantile_s, actions)


@jax.jit
def huber(td: jnp.ndarray) -> jnp.ndarray:
    """Return the Huber loss for the given temporal difference (TD) error."""
    abs_td = jnp.abs(td)
...
class AD_QRDQN:
    ...
    When `forward_connections` is `True`, layers are connected to the
    ...
    Args:
        ...
        num_quantiles: The number of quantiles to use in the QR-DQN algorithm.
    ...
    def __init__(
        ...
        self.num_actions = int(self._env_spec.action_space.n)
        ...
        for i, hidden_size in enumerate(net_arch):
            ...
            layer = layer_cls(
                ...
                self.num_actions * num_quantiles,  # output dimension
                **layer_kwargs
            )
        ...
        # Compute the cumulative distribution function (CDF) of the quantiles
    ...
    def forward(
        ...
        Returns:
            Quantiles for the Q-values for the given observation as an array of
            shape ``(batch_size, num_quantiles, num_actions)``, and a list of
            the activations from each layer with a soft update applied, where
            each activation is a batched 2D array.
        ...
        layer_quantiles, layer_acts = self._forward_jit(params, obs, last_activations)

        if self.average_predictions:
            # Average the first `layer_index` quantiles
            quantiles = jnp.mean(layer_quantiles[:layer_index + 1], axis=0)
        else:
            # Return the quantiles for the given layer
            quantiles = layer_quantiles[layer_index]

        return quantiles, layer_acts
    ...
    def _forward_jit(
        ...
        Returns:
            A list of quantiles for each layer, and a list of the activations
            from each layer with a soft update applied.
        """
        x = self._extract_features(obs)
        batch_size = x.shape[0]
        layer_quantiles = jnp.zeros((
            len(params),  # number of layers to iterate through
            batch_size,  # batch size
            self.num_quantiles,  # number of quantiles
            self.num_actions  # number of actions
        ))
        activations = []
        ...
        for i, layer in enumerate(self._layers[:len(params)]):
            ...
            h, quantiles = layer.apply(params[i], h_in)

            # Reshape quantiles to have separate dimensions for quantiles and actions
            quantiles = jnp.reshape(quantiles, (-1, self.num_quantiles, self.num_actions))

            # Record the outputs
            activations.append(
                alpha * h + (1 - alpha) * last_activations[i]
            )
            layer_quantiles = layer_quantiles.at[i].set(quantiles)

        return layer_quantiles, activations
    ...
    def predict(
        ...
        **kwargs: Any
        ...
        """
        quantiles, activations = self.forward(obs, last_activations, **kwargs)
        actions = jax.device_get(jnp.argmax(quantiles.mean(axis=1), axis=-1))
                ...
                if global_step > self.learning_starts:
                    if global_step % self.train_frequency == 0:
                        ...
                        if global_step % log_frequency == 0:
                            ...
                            if len(ep_infos) > 0:
                                recent_ep_infos = ep_infos[-log_frequency:]
                                metrics['charts/mean_episodic_return'] = np.mean([
                                    ep_info['r'] for ep_info in recent_ep_infos]).item()
                                metrics['charts/mean_episodic_length'] = np.mean([
                                    ep_info['l'] for ep_info in recent_ep_infos]).item()
    ...
    def _local_opt_step(
        ...
        """
        next_h, next_quantiles = layer.apply(state.params, next_x)
        next_h_target, next_target_quantiles = layer.apply(state.target_params, next_x_target)

        # Reshape quantiles to have separate dimensions for quantiles and actions
        next_quantiles = jnp.reshape(next_quantiles, (-1, self.num_quantiles, self.num_actions))
        next_target_quantiles = jnp.reshape(next_target_quantiles, (-1, self.num_quantiles, self.num_actions))

        if self.double_q:
            # Double DQN: select actions for the next states using the
            # online network; a_t+1 = argmax_a Q(s_t+1, a; theta)
            next_actions = jnp.argmax(next_quantiles.mean(1), axis=-1)[..., None]  # (batch_size, 1)
        else:
            # Regular DQN: select actions for the next states using the
            # target network; a_t+1 = argmax_a Q(s_t+1, a; theta^-)
            next_actions = jnp.argmax(next_target_quantiles.mean(1), axis=-1)[..., None]  # (batch_size, 1)

        # Get the quantile values for the chosen actions using the target network
        pred_quantiles = get_quantile_at_action(next_target_quantiles, next_actions)
        ...
        target = rewards[:, None, None] + (1 - dones[:, None, None]) * self.gamma * pred_quantiles
        target = target.reshape(-1, 1, self.num_quantiles)
        ...
        def loss_fn(params: flax.core.FrozenDict[str, Any]) \
            ...
            #
            # The model computes the quantile values for each action,
            # and then we select the corresponding quantile values for the
            # actions taken. These are the actions which would've
            # been taken for each batch state according to the q_net
            h, quantiles = layer.apply(params, x)  # (batch_size, num_quantiles * num_actions)
            quantiles = jnp.reshape(quantiles, (-1, self.num_quantiles, self.num_actions))

            q_pred = quantiles.mean(axis=1)  # (batch_size, num_actions)
            quantiles = get_quantile_at_action(quantiles, actions)

            # Compute element-wise TD loss
            td = target - quantiles  # Temporal difference error
            if self.huber_loss:
                # Use Huber (smooth L1) loss
                el_td_loss = huber(td)
            else:
                # Use MSE loss
                el_td_loss = jnp.square(td)

            el_td_loss *= jnp.abs(self.cum_p_prime[..., None] - (td < 0))  # Mask out negative errors
            td_loss = el_td_loss.sum(axis=1).mean(axis=1, keepdims=True).mean()
